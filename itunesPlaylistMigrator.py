...
try:
    ...
    print(
        "You need to put the IT_file_correlations.py file in the same directory as this script."
    )
    sys.exit(1)
...
def send_api_request(endpoint, **kwargs):
    ...
    try:
        ...
        print(
            f"Could not reach Navidrome Server. You entered {server_url.partition('rest/')[0]}"
        )
    ...
    try:
        ...
        if res["status"] == "ok":
            ...
            print(
                f"Message: {res['error']['message']}. Code {res['error']['code']}."
            )
            return False
    except KeyError:
        print(
            "Seems that the address you entered does not go to a navidrome server."
        )
...
while not login_successful:
    ...
    password = pyip.inputPassword(
        prompt="Enter the password to your Navidrome account: "
    )
...
for plist in playlists:
    if plist.find("key", string="Distinguished Kind"):
        continue  # these are special playlists unique to Itunes

    playlist_name = plist.find("key", string="Name").find_next("string").text
    if playlist_name in playlists_to_skip:
        continue
    if plist.find("key", string="Smart Info"):
        continue
    ...
    should_we_keep_playlist = pyip.inputYesNo(
        prompt="Do you want to move it to Navidrome? "
    )
    if should_we_keep_playlist == "no":
        continue

    create_playlist_reply = send_api_request(
        "createPlaylist", name=playlist_name
    )
    if not create_playlist_reply:
        print(
            "Something went wrong when trying to create the %s playlist."
            % playlist_name
        )
        ...
        ND_track_ids = [
            itunes_correlations[x]
            for x in it_track_ids
            if x in itunes_correlations
        ]

        add_tracks_reply = send_api_request(
            "updatePlaylist",
            playlistId=ND_playlist_id,
            songIdToAdd=ND_track_ids,
        )
